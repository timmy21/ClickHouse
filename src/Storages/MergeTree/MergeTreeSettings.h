...
#define MERGE_TREE_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, part_stale_after_max_time_passed_seconds, 0, "The time threshold in seconds after which a data partition is considered stale based on the max_time field", 0) \
    M(UInt64, number_of_free_entries_in_pool_to_execute_stale_merge, 20, "When there is less than specified number of free entries in pool, do not execute part stale meregs. This is to leave free threads for recent part merges", 0) \
